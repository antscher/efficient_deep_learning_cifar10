...
from pytorch_cifar.models.resnet_fact import ResNet18_fact
...
def apply_fake_quantization(model, k_bits=8, verbose=True, quantize_activations=True):
    """Apply fake quantization with k-bits precision on weights and optional activations."""
    if verbose:
    ...
    activation_hooks = []
    activation_fake_quants = nn.ModuleList()
    if quantize_activations:
        for module in model_quantized.modules():
            if isinstance(module, (nn.Conv2d, nn.Linear, nn.ReLU)):
                ...
                activation_fake_quants.append(act_fake_quant)

                def _activation_quant_hook(m, inp, out, fq=act_fake_quant):
                    if isinstance(out, torch.Tensor):
                        return fq(out)
                    return out

                activation_hooks.append(
                    module.register_forward_hook(_activation_quant_hook)
                )

    # Keep references so hooks and fake-quant modules stay alive.
    model_quantized._activation_fake_quant_hooks = activation_hooks
    model_quantized._activation_fake_quant_modules = activation_fake_quants

    if verbose:
        print(f"  Quantization range: [{quant_min}, {quant_max}]")
        if quantize_activations:
            print(
                f"  Activation fake quantization enabled on {len(activation_hooks)} modules"
            )
        else:
            print("  Activation fake quantization disabled")
...
def pipeline_pruning_quantization_distill(
    ...
    student_model_quant = apply_fake_quantization(
        student_model, k_bits=k_bits, verbose=verbose
    )
...
if __name__ == "__main__":
    ...
    student_ckpt = torch.load(
        "checkpoints/ResNet10_mixup_cos_fact.pth", map_location=device
    )
