...
class BC():
    ...
    def binarization(self):
        ...
        for index in range(self.num_of_params):
            ...
            binarized = torch.where(weights >= 0, torch.ones_like(weights), -torch.ones_like(weights))
            self.target_modules[index].data.copy_(binarized)
    ...
    def clip(self):
        ...
        for index in range(self.num_of_params):
            clipped = nn.Hardtanh(min_val=-1, max_val=1)(self.target_modules[index].data)
            self.target_modules[index].data.copy_(clipped)
