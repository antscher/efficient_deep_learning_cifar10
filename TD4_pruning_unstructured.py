...
def train_model(model, epochs, run=None):
    ...
    for epoch in range(0, epochs+1):
        if epoch == 0:
            print("Evaluate prunung before fine-tuning...")
            acc = evaluate(model, testloader)
            print(f"Accuracy before pruning: {acc:.2f}%")
            if run is not None:
                run.log({"epoch": epoch+1, "test_acc": acc})
            epoch += 1
...
def apply_global_unstructured_pruning(model, prune_ratio=0.5, verbose=True):
    ...
    run = wandb.init(
        ...
        config={
            "learning_rate": 0.001,
            "architecture": f"ResNet18 pr : {prune_ratio} - fine",
            "dataset": "CIFAR-10",
            "epochs": 10,
            "batch_size": 64,
        },
    ...
    if verbose:
        print(f"Saved pruned model to checkpoints/ResNet18_pruned_{int(prune_ratio*100)}.pth")
    torch.save(state, f'checkpoints/ResNet18_pruned_{int(prune_ratio*100)}.pth')
...
if __name__ == "__main__":
    for prune_ratio in [0.6,0.7, 0.8, 0.9, 0.95, 0.99]:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        loaded_cpt = torch.load('checkpoints/ResNet18_mixup_cos.pth',map_location=device)
        # Define the model
        model = ResNet18()
        ...
        model.load_state_dict(loaded_cpt['net'])
        model = model.to(device)
        apply_global_unstructured_pruning(model, prune_ratio)
        acc = evaluate(model, testloader)
        print(f"Accuracy after pruning with ratio {prune_ratio}: {acc:.2f}%")
