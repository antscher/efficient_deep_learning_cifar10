...
from pytorch_cifar.models.resnet_fact import (
    ResNet10_fact,
    ResNet12_fact,
    ResNet14_fact,
    ResNet16_fact,
...
def train_model(model, epochs, run=None, optimizer=None):
    model.to(device)
    ...
    for epoch in range(epochs):
        ...
        scheduler.step()
        print(
            f"Epoch {epoch + 1}/{epochs} - Loss: {avg_loss:.4f} - Train Acc: {acc_train:.2f}% - Test Acc: {acc_test:.2f}%"
        )
        if run is not None:
            run.log(
                {
...
for model_str, optimizer_str, lr in [
    ("ResNet18", "SGD", 0.1),
]:
    # =========================
    # WANDB INIT
    ...
    run = wandb.init(
        ...
        config={
            "learning_rate": lr,
            "architecture": f"{model_str} - Mixup - cos - fact",
            "dataset": "CIFAR-10",
            "epochs": 80,
            "batch_size": 64,
    ...
    match model_str:
        ...
        case "ResNet18":
            model = ResNet18()
    print(f"Model: {model_str} - Optimizer: {optimizer_str} - LR: {lr}")
    print(f"Number of parameters: {count_parameters(model) / 1e6:.2f}M")

    if optimizer_str == "Adam":
        optimizer = optim.Adam(
            model.parameters(),
            lr=run.config["learning_rate"] if run else 0.001,
            weight_decay=5e-4,
        )
    else:
        optimizer = optim.SGD(
    ...
    model = train_model(
        model, epochs=run.config["epochs"], run=run, optimizer=optimizer
    )
    ...
    state = {
        ...
        "architecture": f"{model.__class__.__name__} - Mixup - cos",
        "dataset": "CIFAR-10",
    }
    torch.save(state, f"checkpoints/{model_str}_mixup_cos_{optimizer_str}.pth")
    if optimizer_str == "Adam":
        print(
            f"Model saved to checkpoints/{model_str}_mixup_cos_{optimizer_str}_fact.pth"
        )
    else:
        print(
            f"Model saved to checkpoints/{model_str}_mixup_cos_{optimizer_str}_fact.pth"
